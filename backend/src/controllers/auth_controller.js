...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (user.currentSocketId) {
            console.log(`⚠️ Conflict: Kicking old socket ${user.currentSocketId} for user ${user.username}`);
            const io = getIO();
            io.to(user.currentSocketId).emit("force_logout", {
                message: "Someone else logged into this account. You have been disconnected."
            });
        } else {
            console.log(`ℹ️ No active socket found in DB for ${user.username}. No one to kick.`);
        }
